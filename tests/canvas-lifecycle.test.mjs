...
test("hidden reading controllers are released in source mode while alternate surfaces are cleaned up", async () => {
  ...
  assert.match(source, /if \(isSourceMode\(view\) && sourceVisible && !previewVisible\) \{[\s\S]*controller\.destroy\(\);/s);
  ...
  assert.match(source, /if \(isSourceMode\(view\) && sourceVisible && !previewVisible\) \{[\s\S]*controller\.previewEl\?\.isConnected[\s\S]*controller\.destroy\(\);[\s\S]*controller\?\.destroy\?\.\(\);/s);
