...
test("canvas layers stay hidden until their backing stores are initialized", async () => {
  ...
  assert.match(styles, /\.notedraw-shell\.is-notedraw-preview-shell \.notedraw-underlay-canvas,[\s\S]*\.notedraw-shell\.is-notedraw-preview-shell \.notedraw-static-canvas,[\s\S]*\.notedraw-shell\.is-notedraw-preview-shell \.notedraw-canvas\s*\{[^}]*display:\s*block;/s);
  assert.match(source, /this\.previewEl\.addClass\("has-notedraw-canvas"\)/);
  assert.match(source, /this\.previewEl\.toggleClass\("is-notedraw-preview-shell", this\.surfaceType === "preview"\)/);
