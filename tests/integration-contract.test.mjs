...
test("3.4.84 preserves reading content and cross-view frames without hidden-surface layout writes", async () => {
  ...
  assert.match(source, /repairUnloadedEmbeds\(\)[\s\S]*dest\?\.extension\?\.toLowerCase\?\.\(\) === "pdf"[\s\S]*repairPdfEmbedPlaceholder\(span, target\)/);
  assert.match(source, /repairPdfEmbedPlaceholder\(span, target\)[\s\S]*noteDrawPdfEmbedRepair === "true"[\s\S]*MarkdownRenderer\.render\([\s\S]*host\.querySelector\("\.internal-embed\.pdf-embed, \.pdf-embed\.is-loaded"\)[\s\S]*span\.replaceWith\(rendered\)/);
  assert.match(source, /function isPendingPdfEmbedElement\(element\)[\s\S]*source\.endsWith\("\.pdf"\)[\s\S]*!element\.matches\("\.pdf-embed, \.is-loaded"\)/);
