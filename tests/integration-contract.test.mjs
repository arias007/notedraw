...
test("3.4.84 preserves reading content and cross-view frames without hidden-surface layout writes", async () => {
  ...
  assert.match(source, /restorePendingMarkdownIdentityPresentation\(mutations = \[\]\)[\s\S]*unmatchedMembers\.length === unmatchedDirectCandidates\.length[\s\S]*applyMarkdownBlockFlowPresentation\(block, freshElement\)/);
  assert.match(source, /selectedMarkdownInlineFrameLimitsCanvas\(\)[\s\S]*markdownInlineSelectionClientLimits\(element, rect\)/);
  ...
  const asyncEmbedSource = source.slice(source.indexOf("  readingHasPendingAsyncFileEmbeds("), source.indexOf("  repairAsyncFileEmbedSections(", source.indexOf("  readingHasPendingAsyncFileEmbeds(")));
  assert.match(asyncEmbedSource, /\.internal-embed\[src\], \.internal-embed\[data-src\]/);
  assert.doesNotMatch(asyncEmbedSource, /isPendingPdfEmbedElement/);
  assert.match(asyncEmbedSource, /!element\.closest\?\.\("\.obcc-inline-workbench-embed"\)/);
  assert.match(asyncEmbedSource, /readingAsyncEmbedFilePath = ""/);
  assert.doesNotMatch(asyncEmbedSource, /readingAsyncEmbedFilePath === filePath && filePath\) \{\s*return true/);
  assert.match(source, /syncReadingVirtualSections\(\)[\s\S]*readingHasPendingAsyncFileEmbeds\(renderer\)[\s\S]*restoreReadingVirtualSections\(\)[\s\S]*repairAsyncFileEmbedSections\(renderer\)[\s\S]*return false/);
  assert.match(source, /repairAsyncFileEmbedSections\(renderer[\s\S]*promoteAsyncFileEmbedPlaceholder\(element\)[\s\S]*repairConnectedReadingSections\(renderer\)/);
  assert.match(source, /promoteAsyncFileEmbedPlaceholder[\s\S]*noteDrawAsyncEmbedRepair/);
  assert.match(source, /repairUnloadedEmbeds\(\)[\s\S]*isAsyncFileEmbedElement\(span\)[\s\S]*promoteAsyncFileEmbedPlaceholder\(span\)/);
  assert.match(source, /dest\?\.extension\?\.toLowerCase\?\.\(\) === "pdf"[\s\S]*!isPendingPdfEmbedElement\(span\)[\s\S]*repairNativeFileEmbedPlaceholder\(span, target\)/);
  assert.doesNotMatch(source, /repairPdfEmbedPlaceholder/);
  assert.match(source, /repairNativeFileEmbedPlaceholder\(span, target\)[\s\S]*isPendingPdfEmbedElement\(span\)[\s\S]*isSameEmbedTarget\(element, target\)[\s\S]*span\.replaceWith\(rendered\)/);
  assert.match(source, /function isPendingPdfEmbedElement\(element\)[\s\S]*isPdfEmbedSourceElement\(element\)[\s\S]*!element\.matches\("\.pdf-embed, \.is-loaded"\)/);
  assert.match(source, /function isPdfEmbedSourceElement\(element\)[\s\S]*source\.endsWith\("\.pdf"\)/);
  assert.match(source, /function isSameEmbedTarget\(element, target\)[\s\S]*normalizeVaultPath\(actual\) === normalizeVaultPath\(expected\)/);
  assert.match(source, /function isAsyncFileEmbedElement\(element\)[\s\S]*obcc-inline-workbench-embed[\s\S]*docx\?[\s\S]*pptx\?[\s\S]*xlsx\?/);
  assert.match(source, /isMarkdownContentMutation\(mutation\)[\s\S]*changedElements[\s\S]*embedSelector[\s\S]*changedElements\.every[\s\S]*return false/);
  ...
  assert.match(source, /await this\.prepareFrozenNoteFlowLayout\(\)\.catch\(\(error\) => \{\s*void error;\s*return false;\s*}\);\s*if \(this\.destroyed \|\| generation !== this\.drawingLoadGeneration \|\| this\.file\?\.path !== file\?\.path\) \{\s*return;\s*}\s*this\.syncMarkdownBlockPresentation\(\);\s*this\.resizeCanvas\(\{ layout: false, measure: true \}\);\s*if \(this\.surfaceType === "source"\) \{\s*this\.resizeCanvas\(\{ layout: true, measure: false \}\);\s*}\s*if \(!this\.active && this\.hasNoteFlowElements\(\)\) \{\s*this\.restoreFrozenNoteFlowLayout\(\);\s*this\.scheduleFrozenNoteFlowLayoutRestoreAfterMeasurement\(\);\s*}\s*this\.render\(\)/);
