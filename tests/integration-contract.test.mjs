...
test("3.4.84 preserves reading content and cross-view frames without hidden-surface layout writes", async () => {
  ...
  assert.match(asyncEmbedSource, /isPendingPdfEmbedElement/);
  ...
  assert.match(source, /repairAsyncFileEmbedSections\(renderer[\s\S]*repairConnectedReadingSections\(renderer\)/);
  assert.doesNotMatch(source, /promoteAsyncFileEmbedPlaceholder|noteDrawAsyncEmbedRepair/);
  assert.match(source, /repairUnloadedEmbeds\(\)[\s\S]*isAsyncFileEmbedElement\(span\)[\s\S]*repairNativeFileEmbedPlaceholder\(span, target\)/);
  assert.match(source, /repairUnloadedEmbeds\(\)[\s\S]*dest\?\.extension\?\.toLowerCase\?\.\(\) === "pdf"[\s\S]*repairNativeFileEmbedPlaceholder\(span, target\)/);
  assert.doesNotMatch(source, /repairPdfEmbedPlaceholder/);
  assert.match(source, /repairNativeFileEmbedPlaceholder\(span, target\)[\s\S]*isSameEmbedTarget\(element, target\)[\s\S]*span\.replaceWith\(rendered\)/);
