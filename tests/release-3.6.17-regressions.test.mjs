...
test("view changes synchronize source geometry immediately without projecting hidden reading DOM", async () => {
  ...
  assert.match(onload, /layout-change[\s\S]*if \(!layoutBurstPassScheduled\)[\s\S]*this\.syncMarkdownModeSurfaces\(\);[\s\S]*requestAnimationFrame\([\s\S]*this\.syncMarkdownModeSurfaces\(\);[\s\S]*scheduleSurfaceSync\(120\)/);
